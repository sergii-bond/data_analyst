...
def make_pipeline():
    ...
    pipeline = [{"$match" : {"country" : "India"}},
                {"$unwind" : "$isPartOf"},
                {"$project" : {"city" : "$name", "district" : "$isPartOf", "population" : "$population"}},
                {"$group" : {"_id" : "$district", "district_avg" : {"$avg" : "$population"}}},
                {"$group" : {"_id" : "India Regional City Population Average", "avg" : {"$avg" : "$district_avg"}}}
                ]
    return pipeline
